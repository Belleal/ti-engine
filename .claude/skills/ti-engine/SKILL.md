...
| `utils/cache.js` | `CommonMemoryCache` singleton — owns operational state, connection observation and one shared guard; delegates storage to a `CacheProvider`. `createCacheStore()` (1.17.0) opens an independent, frozen instance beside it |
...
| `components/cache/d1-state-service.js` | The protocol's other end, over D1 (1.17.0, exported as `./state-service`): `createD1StateService( database, { partitions, now } )` → a Fetch handler, `sweepExpired()`. **Self-contained** — no core imports, only Workers/Node globals — so a Worker bundling it pulls in nothing else |
| `components/cache/d1-state-schema.sql` | Its schema, for `wrangler d1 execute --file`; every statement `IF NOT EXISTS`. Not in the exports map (a `.sql` subpath breaks `check:types`, which imports every subpath): locate it beside `require.resolve( "@ti-engine/core/state-service" )` |
| `design/state-protocol.md` | The contract `HttpCacheProvider` speaks, plus the D1 mapping and the shipped implementation (§6) |
...
- **And the service that answers it ships too: `@ti-engine/core/state-service`** (1.17.0, CA-177), with
  **partitioned documents**. The Boris Khan site's Worker answered the protocol from D1 in the site's own repository;
  competence needs the same plus a thing the site never did — an application **collection** in D1. One row per
  document is fatal for that: D1 caps a row at 2 MB, and competence's evaluations (17–21 KB each) reach it at about a
  hundred. A service created with a **partition spec** (`{ key: [ pattern, … ] }`, each pattern segments of `"*"` or
  a literal, no two overlapping) stores each **entity** of a named key as its own `state_partitions` row, while the
  client still sees one document through the same `getJSON` / `setJSON` / `editJSON`. In core, not a new package,
  because npm trusted publishing cannot create a package and `npm-publish-plan.js` refuses every release while any
  workspace package is unpublished. The invariants, each pinned by a test and most measured:
  - **A merge is one `batch()`** — one `json_patch` upsert per entity, nothing read first. D1 runs a batch as a
    transaction, so a multi-entity merge lands whole or not at all.
  - **A subtree read is a primary-key range** (`path >= prefix, < prefix-with-its-comma-raised`), exact under BINARY
    collation. A `substr` comparison visited all 3,000 rows of a key to answer 10, and D1 bills rows read.
  ...
  - **An empty object has no row** (design record §3.4): `{}` written above the entity level reads back `null`, and
    ...
    `*` is refused there; a wildcard anywhere the spec has no `*` is a 400, never a quiet `null`.
  - **Branch reads use `->`, not `json_extract`**, in the single-document path too: `json_extract` answers a string
    unquoted, and the provider parses what it gets — a stored `"42"` came back as the number 42, `"null"` as absent.
    Existence tests use `json_type`, which is NULL only for an absent path. **A path segment holding `"`, `\` or a
    control character is refused** wherever a path expression is needed: SQLite 3.51.2 reads a backslash in a quoted
    label as an escape, and `json_set` wrote a *different* key with nothing raised. A merge needs no path and reaches
    any key.
  The same release adds **`createCacheStore( identifier, { provider, settings, requiredCapabilities } )`** — an
...
- **A screen that never changes is asked for once per release, and every response can say where its time went**
  ...
  process: revalidation saves bytes, not the round trip. `immutable: true` on a fragment content-addresses every
  reference to it and answers the current address `private, max-age=31536000, immutable`, so a revisit makes **no
  request**. The promise is checked per response, not trusted: see the `web-app-manager.js` row. `serverTiming` /
  `TI_WEB_SERVER_TIMING` (off by default) adds `Server-Timing: app;dur=…;desc=…, session;dur=…` to every response.
  **An immutable fragment must never carry anything about its viewer**, because the browser, not the session, keeps it.
- **The login screen has a brand slot, and every string on it is labelled** (1.40.0, CA-179). The brand block — the
  ...
  is absent is simply not spliced. README → *The login screen* lists both slots and every key. The same release
  moved the auth-method gating to the top of `transformHtml`, so a login screen with OpenID alone no longer mints a
  CSRF token (see the CA-166 entry below).
- **The browser is sent less, and nothing twice** (1.39.0, CA-174; design record
  ...
  every screen switch re-sent its fragment, and every stylesheet and script read and re-wrote the session. After it:
  first visit 1,482 → 351 KB, **a refresh 484 → 16 KB**, a sign-in boot 488 → 19 KB, and store operations per refresh
  37 → 11. The pieces:
  - **Labels are a content-addressed bundle.** `/app/config` carries `labelsBundle: { hash, url }`, **not `labels`**;
    `GET /app/labels/<hash>` (unprotected) answers a held hash `public, max-age=31536000, immutable` and any other hash
    with the current catalogue `no-store`. The client fetches it with the browser's **default** cache mode — never
    through `sendRequest`, whose `cache: "no-store"` would download it on every page load — and assigns the tree to
    `configuration.labels`, so `getLabel` and `x-text-label` are unchanged; a payload that still carries `labels` is
    used as-is. `getClientLabels( language )` is the seam to leave out what the browser never reads.
  - **Compression** (the `compression` package, brotli at quality 4 — stated, not "improved": 11 costs tens of ms per
    400 KB on the fly). A view that embeds a CSRF token sets `response.locals.tiNoCompress` and is never compressed
    (BREACH).
  - **HTMX fragments revalidate** (`private, no-cache`, ETag, `Vary: HX-Request`); full pages and token-bearing views
    stay `no-store`. A test must revalidate with `node:http`, not `fetch`: the Fetch standard adds
    `Cache-Control: no-cache` to any request carrying `If-None-Match`, and Express rightly answers that in full.
  - **`/static` and `/.well-known` are served before the session middleware**; every `/static` reference in a served
    fragment carries `?v=<content hash>`, and only a request whose `v` matches the file's current hash is `immutable`
    — the 1.19.0 rule (never `immutable` on a URL that is not content-addressed) holds by construction.
  - The favicon is 7 KB (it was a single 256 px PNG, 75 KB); `/me` is `no-store`.
  - **Not done, on purpose:** per-screen label slices (≤ 15–18 KB once per release, against a declared prefix list
    per fragment), minification, an HTMX history cache (snapshots of screens with personal data in `localStorage`).
- **A CSRF token is minted on demand, never merely because a page was viewed** (1.38.0, CA-166). `csrfInitHandler`
  ...
  lands there) does not; a string token still works. **The sign-in view mints only while its local form survives the
  auth-method gating** (1.40.0, CA-180): `transformHtml` gates first, because the local form holds the view's only
  placeholder. Filled before the gating, an OpenID-only deployment minted, and so stored a session and set two
  cookies, on every anonymous visit to its login screen. A `ti-xsrf-token` cookie with no session behind it (signed out,
