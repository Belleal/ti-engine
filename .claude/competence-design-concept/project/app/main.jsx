...
function App() {
  ...
  const topTitle = ({
    ...
    cycles:           ['Cycles',              'Supervisor · cycle management'],
    'cycle-setup':    ['Cycle setup',         'Competency sets'],
  ...
  return (
    <ToastProvider>
      <div className={`app-shell ${collapsed ? 'collapsed' : ''}`}>
        <Sidebar
          active={
            route.screen.startsWith('eval') || route.screen === 'new-evaluation' ? 'evaluation' :
            route.screen === 'cycle-setup' ? 'cycles' :
            route.screen
          }
          onNavigate={navigate}
          themeName={theme}
        ...
        <div className="content">
          ...
          {route.screen === 'cycles'         && <ScreenCycles            onNavigate={navigate}/>}
          {route.screen === 'cycle-setup'    && <ScreenCycleSetup        cycleID={route.params.cycleID} onNavigate={navigate}/>}
          {route.screen === 'people'         && <ScreenEmployeeManagement scope="supervisor" onNavigate={navigate} initialEmployeeID={route.params.employeeID}/>}
        </div>
      </div>
    </ToastProvider>
  );
