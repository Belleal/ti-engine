...
* feat(state-service): partitioned documents. Created with a partition spec — for a named key, the entity patterns: `[ [ "*", "*" ] ]` for `{ employee: { evaluation: … } }` — the service keeps each entity as its own `state_partitions` row while the client still sees one document through the same `getJSON` / `setJSON` / `editJSON`. One row per document is fatal for an application collection: D1 caps a row at 2 MB, competence's evaluations (17–21 KB each) reach it at about a hundred, and every edit to one evaluation rewrote all of them. A merge becomes one `json_patch` upsert per entity, submitted as one `batch()`, which D1 runs as a transaction, so it lands whole or not at all and two merges into different entities no longer contend. A subtree is read as a range of the primary key: measured on 3,000 rows, the `substr` comparison first written visited every row of the key to answer ten, and D1 bills rows read. A read by id alone (`*.<id>`) uses the `leaf` index — but only with no `ORDER BY` in the statement: with one, SQLite preferred the primary key and scanned the key (measured); the few matches are sorted afterwards by UTF-8 bytes, SQLite's order. And `keys/match` finds partitioned keys through a partial index of the marker rows alone, where it read every entity row of every partitioned key (5,511 for eleven markers, measured). What a caller can observe is stated in the design record and pinned by tests: an empty object above the entity level leaves no row, so it reads back `null` where a single document keeps `{}`; a subtree comes back in path order rather than insertion order; `*` is a wildcard where the spec has one and a 400 anywhere else, never a quiet `null`, and a key spelled `*` is refused on the entity path; a value that does not fit the spec is refused with the path named; `keys/expire` on a partitioned key is refused rather than accepted and ignored.
* fix(state-service): answer a branch read with `->`, not `json_extract`, and test existence with `json_type`. `json_extract` returns a string unquoted, and the provider parses what it is given: a stored `"42"` read back as the number 42 and a stored `"null"` as absent (reproduced before the change). And it is NULL for a stored `null` as for a missing path, so set-if-absent overwrote a `null` it should have kept. Both apply to the single-document path, which the module took from the site's handler.
* fix(state-service): refuse a path segment containing a backslash or a control character, as well as a double quote, wherever a path expression is needed (reading or replacing a branch, writing inside a partitioned entity). SQLite reads a backslash inside a quoted label as an escape: on 3.51.2 a branch `a\b` addressed a different key, so the read answered `null` and the write answered `ok` while adding a second key beside the one it was aimed at (reproduced: the document ended `{"a\\b":1,"a\b":9}`). JSON-escaping the label addresses every key on 3.51.2, but a quote in a label was a bad path on 3.45.1 and D1's version is not pinned, so the segment is refused by name rather than trusted to one version's escape handling. A merge needs no path expression and still reaches any key.
* fix(state-service): decide a set-if-absent or set-if-present in the write it conditions, not in a query ahead of it. Both places that asked first could act on a stale answer. A subtree set above a partitioned document's entity level checked for rows, then ran a batch that deleted the subtree and wrote the new one: when two instances seeded the same collection at once, the second one's seed found nothing, the first one's seed and first evaluation landed, and the second one's batch deleted that evaluation with the rest of the subtree. A set-if-present re-created a subtree deleted in between. The single-document branch set had the same gap between its `json_type` check and its `json_set`: a set-if-absent overwrote a value stored in between, and a set-if-present brought back one deleted in between. Both were reproduced with a fixture hook that lands another request's write just before the unit of work a test picks — a statement run on its own, or a whole batch, the only points D1 lets one land — and each test failed on the old code. The branch set is now one statement per mode, with the condition in its WHERE clause: an `UPDATE` for set-if-present, and an upsert whose `DO UPDATE … WHERE` holds a set-if-absent back, reporting no change (measured on 3.51.2). A subtree set is several statements, so its condition cannot sit in one WHERE clause: the batch's first statement decides it and leaves a row in a new `state_guards` table only if it holds, every write after it runs only while that row exists, and the last statement removes it, so the table is empty between batches. A subtree set without a condition sends the batch it sent before. Two alternatives were rejected. Ordering the statements so the marker is written last would work at the root only, since nothing else marks a subtree. A first statement that aborts the batch would work everywhere, but the service could tell a skip from a failure only by matching D1's error text. Nine hand-made defects across this fix and the two below — each guard condition dropped or inverted, the skip reported from the wrong statement, each branch condition dropped, the bracket unescaped, the token inherited again — each failed 1 to 11 tests.
* fix(state-service): match `keys/match` patterns with `GLOB`, not `LIKE`. `LIKE` folds ASCII case, so `a_b` matched `A_b`, where Redis and the protocol match case-sensitively and the stub server always did. And no index on the key can serve a comparison that folds case: by `EXPLAIN QUERY PLAN`, the `LIKE` statement read every live row of the three tables through their expiry indexes and scanned every marker, where `GLOB` reads the primary-key range that a pattern's literal prefix names in each of the four. A `[` is written as the class that holds only itself, since it opens a class in `GLOB` and is an ordinary character in the protocol. `globToLike` is now `toSQLiteGlob`.
* fix(cache): a store that names its own `stateUrl` sends only the `stateAuthToken` given beside it, and never inherits the configured one. `HttpCacheProvider` fell back to the configuration for every setting a store left out, the token included, so a store opened on another service sent the configured service's bearer token to that address. Reproduced with the token configured: the stub behind a store's own address received `Bearer configured-secret`. `stateAllowInsecureAuth` follows the same rule, since the configured exemption vouches for the configured service's transport only. The timeout and probe interval still fall back, since they carry no credential.
* feat(cache): open an independent store with `createCacheStore( identifier, { provider, settings, requiredCapabilities } )` (CA-176). `CommonMemoryCache` was a singleton over the one configured backend, so an application's records had to live wherever the framework's sessions did. A store is a frozen `CommonMemoryCache` with its own backend, connection observation and capability check, beside the configured cache and never instead of it; `HttpCacheProvider` takes its state settings as an optional second constructor argument (`TiHttpCacheSettings`), so the store names its own service.
* feat(cache): read a JSON value in one shape from either backend with `getJSONValue( key, path )` (CA-178). `getJSON` answers RedisJSON's list of matches from Redis (`[ value ]`, `[]`, or `null` for a missing key) and the value itself over HTTP, and the `result[ 0 ]` unwrap every caller wrote is right for the first and truncates a stored array on the second — measured in competence, where a competency baseline read back as its first code and then as empty. `getJSONValue` answers the addressed value or `null` on both; `getJSON` is unchanged and its documentation now states the difference. A wildcard is RedisJSON's feature, not the protocol's: Redis answers its first match anywhere (what the unwrap gave), while over HTTP a segment is a literal key, except at an entity position of a D1 partitioned document, which answers the first match and refuses a `*` anywhere else in that document. The documentation states each case rather than promising one behaviour.
* test(state-service): 82 tests against real SQLite through `node:sqlite` shaped as a D1 binding, whose `batch()` is one transaction and which logs every statement it runs: the protocol (29), partitioned documents (35), a differential suite (6) and stores end to end through `HttpCacheProvider` (12). The differential suite runs competence's own DataManager operation sequence and 7,500 seeded random operations against a reference that keeps each key as one document, comparing every answer; eight hand-made service defects (a null that patches instead of deleting, a subtree delete dropped, set-if-absent tested against the marker, the last wildcard match, a merge that replaces, set-if-absent ignored, `{}` for an empty subtree, a subtree set that keeps what it replaced) each failed 4–6 of its 6 tests. The SQLite suites skip on a Node before 22.5; with `node:sqlite` blocked, the 11 that need no database still run. The differential suite drives 405 guarded subtree sets, 205 of which went ahead and 200 were skipped. Core 139 to 222 tests (one of them the new fixture module, which `node --test` runs like the other three).

## Version 1.16.0

* feat(config): switch the health heartbeat off from the environment with `TI_SERVICE_HEALTH_CHECK_ENABLED` (CA-168). 1.14.0 made the heartbeat optional through `serviceConfig.healthCheckEnabled`, but core reads that only from its own `bin/settings.json`, which a consuming package cannot override, and gave it no environment variable - so no application could actually turn it off. A single instance with nothing watching it wrote one every second regardless: on the Boris Khan site, a D1 write a second for as long as its container was awake, and the only cache call core itself still makes once the exchange is off. The test boots a real consumer with the switch off, on state over HTTP, and asserts no heartbeat reaches the state service in over a second. It fails with the override removed, and it fails with the switch on.
* fix(cache): say why the state service health probe failed (CA-167). `HttpCacheProvider.initialize()` built its message from `error.message`, but `#fetchJSON` rejects with a raised TiException, which keeps its account in `data.details` and has no `message` - so every failure read `did not answer the health probe: undefined`. That cost a full investigation on the Boris Khan site's first deploy, where the container could not resolve its state hostname and the one line that should have said so said `undefined`. Transport failures now also carry what `fetch` put in `cause`, because `fetch` says only "fetch failed" for a DNS failure and a dropped socket alike; the line now reads, for example, `fetch failed: getaddrinfo ENOTFOUND state.internal`. The status case and the transport case each have a test, and both fail on 1.15.2 with the exact production message. Core 124 to 127 tests.
* feat(auditing): give each JSON log line a `level` name beside its numeric `severity` (CA-171). The JSON mode was shaped for GCloud, which reads the level from `severity` - the enum is GCloud's own LogSeverity. Cloudflare's container logs read neither that number nor the stream: on the Boris Khan site's staging deploy every line showed as `info`, including a DEBUG entry and two WARNING entries written to stderr. Cloudflare's own events carry their level as a plain `level` field, so the line now does too: `debug` below INFO (DEFAULT included: it sits below DEBUG, and the minimum level the README recommends for production drops both), `info` for INFO and NOTICE, `warn` for WARNING, and `error` from ERROR up. The field is added to the console line only, not to the log entry. `severity` and the stderr split are unchanged, so GCloud reads the line as before, and text mode is untouched. Cloudflare does not document which field it reads for container output, so whether `level` takes effect is confirmed on staging once this release reaches the site. 12 tests: 9 fail without the field, and 3 pin what must not change (`severity` and the rest of the entry, the stderr split, and one line per entry even with a multi-line stack in its data). Core 127 to 139 tests.
