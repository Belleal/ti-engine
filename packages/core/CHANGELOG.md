...
* test(service-consumer): start and stop a real ServiceConsumer and ServiceProvider with the exchange off, on state over HTTP. One file each, because only one ServiceInstance may exist per process. Each verified to fail with its gate removed. Core 116 to 122 tests.
