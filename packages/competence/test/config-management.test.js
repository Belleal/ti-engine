...
describe( "config-registration (competence)", () => {
    ...
    it( "keeps competence-labels' default reading the directly-required labels constant, and pins a default for every store-backed document", () => {
        ...
        // The complementary half: every one of the nine STORE_BACKED documents must register ITS OWN file default —
        ...
        // sides are still "a defined value", so a weaker assertion would pass every one of these nine checks even
        // though the wrong document is being registered.
        for ( const key of [ "competencies", "relevancy-archetypes", "active-competency-sets", "role-families", "role-family-competencies", "stage-levels", "research-consent", "organization-structure", "work-sites" ] ) {
