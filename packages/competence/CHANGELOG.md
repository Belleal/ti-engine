...
* feat(config): create the `TC` role family with the `TECHNICAL_WRITING`, `DOCUMENT_COMPLIANCE` and `KNOWLEDGE_MANAGEMENT` specializations, and retire `BA.DOC_PROC`. Adding a family touches **nine registration changes across eight files beyond the content** — `role-families.schema.json` (required + properties), the family-code patterns in `cycle.schema.json`, `role-family-competencies.schema.json` and `active-competency-sets.schema.json`, the `roleFamily` enums in `employee.schema.json` and `evaluation.schema.json`, `roleFamilyCodeEnum` in `configuration-loader.js`, and the `RoleFamilyCodeValue` typedef. Only the first was caught by the test suite; the employee and evaluation enums would have failed silently until the first real TC employee was created
* fix(config): register `TC` in the `ConfigRoleFamilies` typedef, which enumerates families individually and was the one registration point missed on the first pass
* docs(competence): record the upgrade consequence, verified against the schema — an instance seeded before 3.31.0 keeps its pre-TC `role-families` document, because `seedIfEmpty` writes the file default only into an empty store, and that document now fails validation with `should have required property 'TC'`. Reads are unaffected, but **the Configuration screen cannot save an edit to Role Families until the document's drift is reconciled** (Administration → Configuration, which writes the current file default as a validated change-set). Every future family addition will create the same obligation
...
family. PD is the second consecutive family to draw the shared canonical core **without modification and without
contributing anything new to it**: MC established that the canonicalization principle survives leaving ICT, and PD
shows the same outcome for a discipline that is neither engineering nor communications, so **no other family's pool
changed**. The load-bearing scoping decision is the boundary with BA — PD owns *what to build and why* (direction,
value, prioritisation, commercial outcome, looking outward to market and customer) while BA owns *what precisely and
how specified* (elicitation, analysis, requirements, solution definition, looking inward to delivery). They overlap
at junior level and diverge sharply at senior level, which is the argument for two families rather than one. Purely
additive: no evaluation data migration (CA-115).

* feat(config): add the 23 PD family-specific competencies — E1-85…E1-93 (product strategy and vision, market and competitive analysis, customer and user needs analysis, product lifecycle management, prioritisation frameworks and value assessment, product economics and commercial models, product roadmapping, public-sector product and service context, product metrics and success measurement), E2-100…E2-107 (defining and communicating direction, market and customer research, roadmaps, prioritisation and backlog, business cases, customer and stakeholder relationships, performance measurement, product launches), E3-40…E3-42 (accumulated experience, tooling, deciding under uncertainty) and I1-20…I1-22 (process adherence, product and roadmap review, documentation and decision-record standards)
* feat(config): assign relevancy archetypes for all 23 (A ×4 · B ×13 · C ×1 · D ×1 · E ×4). **`E1-92` sits on B rather than C**, which is where the obligation principle gets its boundary: MC's `E1-83` is on C because procurement rules, political neutrality and transparency duties bind every level equally, whereas the public-sector *product context* — public value reasoning, policy as a requirement driver, institutional alongside citizen users — is contextual knowledge that deepens with seniority, consistent with PM's `E1-44`/`E1-45` and BA's `E1-46`/`E1-47`. The principle applies where there is an actual obligation, not wherever the words "public sector" appear. PD's pool resolves to 59 competencies (23 family-specific + 36 shared)
* feat(config): add the PD baseline active competency set for the `2026-H2` cycle — 23 codes satisfying nine-subcategory floor coverage within the cap of 32, composed as direction → customer value → prioritisation → public-sector context on the E1 side and research → direction → roadmap → backlog → measurement on the E2 side. The four PD specializations are left **absent** rather than empty. A cycle seeded from a **fresh or drift-reconciled** configuration includes PD automatically, since the seeder derives `excludedFamilies` at creation time — on an upgraded instance the seeder reads the store-backed active sets, so until the drift is reconciled it still sees the pre-PD document. An **existing seeded** cycle keeps its original exclusion list, because `cycle.excludedFamilies` is derived once and deliberately never re-derived (CA-103), and Cycle Setup flags it as a stale exclusion so a Supervisor can include PD deliberately; a cycle **created through the UI** starts with an empty exclusion list and so includes PD already, though that also means lock validation's `family-not-configured` rule will fail on `IO` until it is populated or explicitly excluded
...
* docs(competence): record Increment 7 in the competency change log, including the BA boundary, the two meanings of Product Ownership across BA and PD, and the `E1-92` archetype reasoning; add the PD section to the master index (totals 232 → 255, PD removed from the unpopulated list) and re-derive the relevancy model's distribution table (258 assignment rows resolving to 255 distinct competencies)
* build(release): bump package version from `3.29.0` to `3.30.0`
...
* feat(config): add the 26 QE family-specific competencies — E1-48…E1-57 (testing principles and levels, test design techniques, quality models, risk-based testing, defect management and root cause analysis, testability analysis, test data management, automation concepts and architecture, non-functional testing concepts, lifecycle quality assurance), E2-42…E2-51 (test planning and strategy, writing test cases, execution and reporting, exploratory testing, defect reporting and triage, regression and suite maintenance, API and integration testing, data validation testing, test environments, quality metrics), E3-28…E3-30 (accumulated QE experience, QE tooling, complex defect diagnosis) and I1-8…I1-10 (QA process adherence, requirement and design review participation, test documentation standards). The dictionary goes from 108 to 134 competencies
* feat(config)!: relocate `E1-10 Business and IT domain knowledge` from BA family-specific to shared canonical, in a new "E1 (cross-cutting)" group, and reword its description and its **R** and **T** anchors away from analysis-specific framing (`typical analysis` → `typical work`; `aligns analysis with` → `aligns the team's work with`). The name and the N/J/S/X anchors are unchanged. Because the per-family pool is derived from section membership in `design/competency-relevancy-model.md`, the move adds E1-10 to **every** family pool: SE 61→62, PM 55→56, BA 52 (unchanged — it already had it), and the five unpopulated families 30→31. No stored evaluation references change, but a consumer that treated E1-10 as BA-only will now see it offered to every family
* feat(config): assign relevancy archetypes for all 26 QE competencies (A ×2 · B ×8 · C ×3 · D ×1 · E ×10 · F ×2), derived to match how the SE/BA/PM sets were assigned — foundational knowledge and daily tooling on A, judgment/strategy/experience on B, process-and-documentation discipline on C, the one early-heavy execution mechanic on D, the core "doing" competencies on E, and the two expert-leaning depths (automation architecture, complex defect diagnosis) on F. QE's pool resolves to 57 competencies (26 family-specific + 31 shared)
* feat(config): add the QE baseline active competency set for the `2026-H2` cycle — 22 codes satisfying nine-subcategory floor coverage within the cap of 30, shaped to match the existing SE/BA/PM baselines. The four QE specializations are left **absent** rather than empty, since the `no-empty-baseline` lock rule only engages once a family carries specialization data. The seeded cycle derives `excludedFamilies` from which families have competencies, so QE is now automatically included in that cycle
* feat(localization): add English and Bulgarian name, description and all six scope anchors for the 26 new competencies (416 strings), and revise the E1-10 description and R/T anchors in both languages
* docs(competence): add `design/competency-change-log.md` — the increment-tracking document for competency content after the v1.0 baseline, so future families land as precise deltas rather than repeated full rebuilds. Its Increment 1 config-impact table is corrected against the current config: the increment was drafted against `config.competency-relevancy.json`, which was retired in 3.1.0 in favour of the archetype model plus the per-family pool
* docs(competence): update the master index (QE section, E1-10 moved to Shared Canonical, totals 108 → 134) and the relevancy model doc (QE assignment section, E1-10 row moved to Shared). Correct the model doc's distribution table, which disagreed with its own assignment tables on nearly every row and claimed a total of 113 against an actual 134; replace its stale "Materialization" section, which still described emitting the retired `config.competency-relevancy.json`; and fix its calibration contract, which still promised that the emitted per-family file lets a later pass diverge an individual family's curve "without structural change" — that file holds pools only, so per-family divergence is precisely what would need a per-family weight file reintroduced
* test(competence): repoint four fixtures that used QE as their stand-in for an unpopulated family — `getActiveCompetencySet` missing-baseline, lock normalization of an excluded family, and the `no-empty-baseline` and `family-not-configured` lock rules — onto XD, which is still unpopulated. The assertions are unchanged; QE simply stopped being an example of an unconfigured family
* build(release): bump package version from `3.16.0` to `3.17.0`

## Version 3.16.0

The app was containerized and continuously delivered, but there was still nowhere for anyone other than a developer to open it. This adds a hosted test environment on Google Cloud Run that costs approximately nothing when idle: a single instance holding the app plus a `redis:8-alpine` sidecar, with Redis snapshotting onto a mounted Cloud Storage bucket so that cycles, evaluations and feedback are *intended* to survive scale-to-zero — the snapshot-to-object-storage path is unverified against a live deployment, and INSTALL.md Method D documents the loss window it carries. Identity-Aware Proxy fronts it with an email allowlist, and the app itself is Google-sign-in only. Setup and deployment are idempotent, dry-runnable scripts rather than a wiki page (CA-94).
...
* feat(competence): populate `workflow.selfEvaluationDeadline` / `managerEvaluationDeadline` at evaluation creation from the cycle's dates (mirroring the existing `teamEvaluationDeadline`), activating the previously dead late-submit guards for the self round and the deadline data the new manager-round nudge and oversight screen rely on
* feat(competence): add `backfillMissingEvaluationDeadlines` — an idempotent framework migration, invoked once at service start (`bin/competence-web-server.js` `onStart`), that stamps the same cycle-derived deadlines onto pre-existing `Open`/`In Review` evaluations left with empty `""` deadlines from before this feature; it only fills an empty field (never overwrites a set one, so re-running is a no-op), never touches `Closed`/`Deleted` evaluations, and skips (with a debug log) an evaluation whose cycle cannot be resolved rather than failing startup. No audit entries are written (a system migration, not a user action). After it runs, every existing consumer (the late-submit guards, `finalizeSelfEvaluation`, the overdue dashboard tasks, Oversight's `canAdvanceSelf`) works unchanged on legacy evaluations too, so no read-time fallback is needed
...
* fix(competence): exclude a team round finalized with zero submissions (`allowFinalizeTeamWithoutSubmissions`) from the score renormalization — such a round is marked complete but carries no grades, so counting its 0.30 weight depressed an all-R self/manager result to 70; `teamParticipates` now also requires `teamEvaluationsSubmitted > 0` (CodeRabbit review, PR #91)
* fix(competence): persist a `workflow.selfEvaluationWaived` flag when a Supervisor advances a stalled self round — a repeat waiver is rejected, and both `Open → In Review` transitions now treat `selfEvaluationCompleted || selfEvaluationWaived`, so a self round waived while the team round is still pending advances on team completion instead of re-stalling; scoring stays keyed on `selfEvaluationCompleted`, so the waived self remains excluded (CodeRabbit review + final-review finding, PR #91)
* fix(competence): use `configurationLoader.evaluationStatus` constants (not `"Open"`/`"In Review"` string literals) in the `overdue-self`/`overdue-manager` task-resolver checks, matching the rest of the file (CodeRabbit review, PR #91)
...
* fix(competence): stop leaking the manager's written feedback and the raw anonymous peer comments to the employee — `anonymizeEvaluationScores` left `feedback` intact for the EMPLOYEE role, so the employee's `load-evaluation` (and save/submit/results) payload carried `feedback.managerComment` from the moment the manager drafted it (before results are revealed — inconsistent with the manager *grade*, already withheld until READY/CLOSED) and the raw `feedback.teamComments` (anonymous peer free-text the employee must never see; only the team cumulative grade is shown). For the employee, `managerComment` is now withheld until READY/CLOSED (mirroring the grade reveal) and `teamComments` is always emptied; the manager keeps full visibility. Latent until the capture fix above populated those fields (CA-89)
* test(competence): add `test/fragment-input-bindings.test.js` — guards that fail on (a) any `@ti-input`/`x-on:ti-input` handler in a fragment (the event is never dispatched, so such a binding is always dead) and (b) an editable feedback textarea binding its value to the placeholder getter `getFeedbackComment`; covers both capture defects and the CA-85 class so they cannot recur. Extend `test/competence-framework.anonymize.test.js` with a role×status matrix for `anonymizeEvaluationScores` feedback exposure (CA-88, CA-89)
