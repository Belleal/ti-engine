...
- Supervisor (holds `SUPERVISOR`, not an org-line superior of the employee) — permitted, but the **manager submit** requires a non-empty `reason` (`error.evaluation.reason-required`) and writes `appendAuditEntry({ subjectType: "evaluation", subjectID, changedBy: userID, field: "grades.managerProxy", newValue: { by: "supervisor" }, reason })`. The manager late-submit guard now fires for an ordinary past-deadline manager submit, but the authorized recovery submit (org-line superior, or Supervisor-with-reason) **bypasses** that guard — completing a past-deadline manager review is the entire purpose of the escape. The bypass is scoped to the authorized recovery path only.
