...
- **Migration — forward-only, no read-time fallback implemented.** Evaluations created before this change carry `""` deadlines, and the code reads the raw `workflow.selfEvaluationDeadline`/`managerEvaluationDeadline` everywhere (the guards, the finalize preconditions, the overdue tasks) — an empty string is simply falsy, so a legacy `OPEN`/`IN_REVIEW` record is never deadline-enforced and never surfaces as overdue. This is **not** the `teamEvaluationDeadline` shape: that field is populated with a fallback *at write time* in `createNewEvaluation` (`:546`); there is no equivalent *read-time* fallback for the new self/manager deadlines, and none is implemented by this feature. A stalled legacy evaluation is recovered the same way any stalled evaluation is — Supervisor `withdrawEvaluation` + a fresh `start-evaluation`, which snapshots current cycle dates. A read-time fallback/backfill for legacy evaluations (resolving `cycle.teamFeedbackDeadline`/`cycle.cycleDate` when the stored deadline is empty) is deferred **future work** (see the Implementation log correction below and §12).
