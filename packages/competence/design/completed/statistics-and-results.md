...
### Phase 4 — Cross-cycle trends (CA-70, 2026-06-25)

Understand pass (`wf_19a881c9`) found the snapshot **read** side fully built (data-manager accessors + write-on-close) but the **compute** side PARTIAL: `buildResultsSnapshot` left six substrate fields as empty `{}` placeholders and there was no cross-cycle read method. So Phase 4 first **completes the deferred substrate** (was lock-shape-only in CA-64) before the trend endpoint/screens. Plan: `docs/superpowers/plans/2026-06-25-stats-results-phase-4-cross-cycle-trends.md`. Scope decisions: trends are **SUPERVISOR-gated** whole-org analytics (matching the org-scope leadership reports); the composition charts reuse the existing **bars stacked** mode (one bar per cycle) rather than a new stacked-area mode; the provisional live ACTIVE-cycle point and manager-subtree trends (via byOrgUnit slices) are noted follow-ups.

- 2026-06-25 — **X0 substrate LANDED (`ea7a8ec`).** `buildResultsSnapshot` now computes, over the reported subset: `overall.finalScore` (five-number + mean/population-stdev) + `tBandMix`; `byCategory`; `ladderOrdinalHistogram` (N→1,J→2,R→3,S→4,**X+T→5**) + `ladderMeanRung`; `byRoleFamily`/`byOrgUnit` cohort cells, **n<MIN_COHORT_SIZE suppressed** (§5.3). CohortRow gains per-category `scores` + resolved `organizationUnitName` (new `OrganizationManager.resolveOrganizationUnitName`). **`schemaVersion` 1→2; snapshots are never back-fillable** so readers tolerate the legacy v1 empty shape.
- 2026-06-25 — **X1 `ti-chart-line` primitive LANDED (`c01b6c4`).** Pure `lineLayout` (categorical x, y-domain over values+band, null gaps break segments) + `renderLine` (band area, polyline-per-segment, dots, x-labels, sr-table); `style:"dashed"`/`provisional`/`provisionalLastPoint` (dashed final connector for the live cycle); sparkline mode; 640px cap + `.ti-chart-line-*` tones. Verified in the preview harness both themes. Stacked-area deferred (X3 reuses bars stacked).
- 2026-06-25 — **X2 read layer LANDED (`573d91e`).** Pure `_computeTrendWith(deps, params)` (mirrors `_resolveWith`) shaping one metric — overallScore (line+band), tBandMix/ladder (stacked), gapClosure (9 subcategory sparklines), cohort (per role-family/org-unit/stage-level mean) — from chrono-sorted snapshots. Tolerates legacy (null + `legacyCycles`, never throws) and skips n<k cohort cells (`suppressedCycles`). `computeTrend` wires `getAllResultsSnapshots`; `#loadResultsTrend` (SUPERVISOR) exposes `load-results-trend`.
- 2026-06-25 — **X3 Trends screen LANDED (`a87eae1`).** `configureTrendsScreen` + `frame-insights-trends.html` + Insights sidebar entry (hasRole 3) + en/bg labels with methodology blocks. Four cards (overall+tband, gap-closure, ladder+rung, cohort). Verified: 21 label keys resolve; the factory transforms produce valid specs from real `_computeTrendWith` output; new chart shapes render in the harness. Full in-app screen verification needs a backend run (deferred).
- 2026-06-25 — **X4 per-employee history LANDED (`945cc4d`).** Pure `buildEmployeeHistory` (reported-only chronological finalScore line, `{noHistory:true}` below two cycles) + `#loadEmployeeHistory` (access-gated: self / Supervisor / manager-of-subtree via `isSuperiorManagerOfEmployee`, mirroring the alignment-drill gate; reads RAW evals, **never snapshots**). Surfaced on "My results" as a `hasHistory()`-gated line card.
- 2026-06-25 — **Phase 4 adversarial review (`wf_bac48e10`) — 3 minor/nit findings FIXED (`a09e2b8`); all critical/major surfaces (byRoleFamily/byOrgUnit suppression, X4 IDOR gate, X3 CSP/labels, legacy tolerance) verified CLEAN.** `byStageLevel` cohort cells now carry `{n,suppressed:true}` below the floor (parity with byRoleFamily/byOrgUnit, so a stageLevel cohort trend flags small cells in `suppressedCycles`); the `gapClosure` value map gained the `isLegacy` guard its sibling metrics use. +1 suppression test. 229 competence + 109 ti-charts tests green, eslint clean. **Phase 4 complete → the Statistics & Results capability (Phases 0–4) is ready to release.**

### Phase 5 — Individual-results UX refinement (follow-up, 2026-06-29)

Owner review of the live evaluation **individual results** view (Phase 3) flagged that the section was visually heavy and under-explained, plus two context bugs. Root causes found by a 6-dimension adversarial review (`wf_1203cce9`, 47 confirmed findings): the cards used a `ti-card-title` class with **no CSS rule** (so every heading fell back to an oversized default `<h2>`) and a bare `.ti-card` with **no padding**; the grouped source bars and the radar rendered **no legend**, and the category bars showed no value labels; the hero stat showed the raw threshold label **key**; the panel header copy was fixed second-person; and strengths vs gaps displayed **inconsistent quantities** (absolute mean vs signed gap).

- 2026-06-29 — **Phase 5 LANDED (web-framework 1.12.0 + competence 3.7.0).** **Charts (`ti-charts.js`, TDD):** `_renderBarsGrouped` gains `options.legend` (swatch legend) + `options.valueLabels` (per-bar value caption, reserving an 86-unit track so labels stay inside the 100-wide viewBox); `renderRadar` gains `options.legend`; `_buildChartLegend` supports a `{ dashed:true }` outlined swatch for the dashed "expected" curve; new `.ti-chart-bar-seg.tone-info` / `.ti-chart-legend-swatch.tone-info` / `.is-dashed` CSS so source series share one colour scale. 61 ti-charts / 112 web-framework tests green. **Results view (competence):** the section is now a lean three-band `ti-card-grid` of `ti-panel competence-panel-body` cards — (1) a full-width hero merging the final-score stat tile with per-category **score chips** (band-tone fill bars, replacing the stretched bar chart that ballooned in a wide column), (2) the subcategory radar and the source-comparison bars side by side, each with a "what it shows" intro + a "How it's calculated" disclosure, (3) strengths | development areas (both as the signed deviation-vs-expected with semantic colour) and the full-width score-history line. **Fixes:** `buildResults` resolves `finalScore.interpretationName` through `getLabel`; new role-aware `getResultsTitle`/`getResultsDesc` getters (manager/team viewer → "Results" / "…for {name}"); score-history now loads for an authorized manager/supervisor (passes the viewed `employeeID`; the endpoint already gates access), not only "My results". New en/bg labels (role-aware title/desc, per-chart methodology + intros, `vs expected`, distinct empty-state strings). Verified in a render harness against the real CSS + `ti-charts.js` (legends/value-labels/dashed-expected present, hero height ~470px→233px, chips tone-coloured, deltas green/red, no console errors). 274 competence tests + JSON validation green, eslint clean.
- 2026-06-29 — **Phase 5 owner-review tweaks LANDED.** (1) **True score ceiling:** the score bars/hero now fill against `ceil(gradeWeights.S × Σ participating-source-weights × 100)` (=130 at full participation; a missing source drops its weight), computed in `buildResults` from config — not the arbitrary T5=150. Investigation of `calculateFinalEvaluationScores` confirmed the category score is a **relevancy-normalised** weighted average (`Σ(w·rel)/Σrel` per source, blended by source weight, ×100), so the maximum is **family/competency-independent** (the relevancy divisor cancels) — documented inline. (2) **Identity colours:** per-category chips now use the E/I/C letter-box colours (shared `--competence-cat-e/i/c` vars, reused by the grading-form letter boxes); the source bars + radar use the SELF/MANAGER/TEAM grading-column colours (self=`--info`, manager=`--accent`, team=`--success`) via app-level `tone-self/manager/team` chart-tone classes in `competence-main.css` (the generic `ti-charts.js` stays colour-agnostic). This fixes the prior manager/team colour collision (both blue). (3) **Expected curve** is now a distinct dashed `--fg-secondary` grey (was the accent red, which now collides with manager) so it reads as a benchmark and is clearly visible. (4) **Equal-height band 2:** both the radar and source cards carry `.competence-results-card-fill` (`align-self:stretch` + a flex-centred figure) so they match height and the shorter card's chart centres into the gap. Verified in the harness at 1280px (two 560px columns, both cards 652px, radar centred) and at 531px (single-column collapse). 274 competence + 112 web-framework tests green, eslint clean.
- 2026-06-29 — **Phase 5 second review pass LANDED.** From a follow-up screenshot: (1) **radar axis labels** (E1…C3) are now category-coloured — `ti-charts.js` radar gained an optional per-axis `tone` (threaded through `radarLayout`, applied as a `tone-*` class on the label), and `buildResults` passes `tone: "cat-"+letter`; competence CSS maps `tone-cat-e/i/c` to the shared `--competence-cat-*` vars. (2) **Legend placement** — the `.competence-results-card-fill` figure was a *row* flex container, which pushed the appended legend beside the chart and collapsed the grouped-bars SVG (the "hidden chart"); switched to `flex-direction: column` so the legend (and sr-table) stay below the chart and the SVG keeps full width, while still centring vertically. Fixes both the radar legend-on-the-right and the source-bars overflow/clip in one change. (3) **Thinner radar strokes** (0.8→0.4, expected 0.5) for the nine-axis profile, scoped to the results radar. Re-verified in the harness at 1280px: axis labels purple/teal/rust, both legends below their charts, source SVG 518px in a 560px card (not collapsed), strokes 0.4/0.5px, cards equal height. +1 ti-charts test (axis tone). 274 competence + 113 web-framework tests green, eslint clean.
- 2026-06-29 — **Phase 5 third review pass LANDED.** Owner asked for thinner self/manager/team comparison bars. `ti-charts.js` grouped bars gained optional `options.barThickness` (→ `barsGroupedLayout` `barH`) and `options.valueFontSize` (value-caption font-size attribute); value captions now also carry a `ti-chart-bar-value` class. Defaults unchanged (barH 4, CSS font). `buildResults` source spec passes `barThickness: 1.33` (≈⅓ of 4) + `valueFontSize: 2.2`. Verified in the harness: bar height 1.33 viewBox (6.9px, ⅓ of the prior 20.7px), value font 2.2 (11.4px), 9 bars/9 labels, svg full-width, no overlap, no console errors. +2 ti-charts tests (barThickness/valueFontSize + defaults). 274 competence + 115 web-framework tests green, eslint clean.
  - **Follow-up fix:** `valueFontSize` had no visible effect — the caption carried `ti-chart-bar-label` whose CSS `font-size: 4px` overrode the `font-size` presentation attribute (a CSS rule always beats a presentation attribute). Confirmed via `getComputedStyle` (attr 2.2 but computed 4px). Fixed by giving the caption its own `ti-chart-bar-value` class that sets only `fill` (no CSS font-size), so the attribute governs (default 4); re-verified computed font-size = 2.2px and fill = ink grey. Lesson for the verification harness: check `getComputedStyle`, not just the attribute, and cache-bust the stylesheet on reload.
