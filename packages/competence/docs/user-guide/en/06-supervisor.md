...
|---|---|---|
| Advance without self | The self-assessment deadline has passed and the round is still incomplete | Waives the stalled self round — those grades are excluded from scoring, never treated as a zero — and moves the evaluation on once the peer review is also done |
