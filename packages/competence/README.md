...
- **[UI]** Administration screens (admin-allowlisted users) — Configuration landing (config change feed, validated restore, export-to-git bundle), Competency Text Editor (bilingual, for BG review), Archetype Assignment, Archetype Curve Editor, Role Families editor, Work Sites editor (the nomenclature backing the employee work-site field), and Employee Import (CSV upload with a previewed, irreversible apply — the one screen here that writes employee data rather than configuration)
