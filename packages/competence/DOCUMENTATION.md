...
3. The `Employee` receives a notification in the system (additionally, an email or a messanger memo depending on the configuration). They are required to fill in the self-evaluation `Grades` in the `Evaluation` form for all `Competences` up until a specified date. Once ready, they need to submit the form as completed by them.
