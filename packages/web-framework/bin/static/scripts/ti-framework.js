...
/**
 * Compute the visible bounding box for positioning UI elements.
 *
 * @param {boolean} isFixed - If true, coordinates are relative to the viewport; if false, coordinates are relative to the document (include scroll offsets).
 * @returns {{left: number, top: number, width: number, height: number, pageLeft: number, pageTop: number}} An object with pixel measurements: `left`/`top` are the box origin, `width`/`height` are its size, and `pageLeft`/`pageTop` are the origin in document coordinates.
 */
