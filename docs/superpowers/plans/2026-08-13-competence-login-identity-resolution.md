...
- [ ] **Email identity resolves.** *(Local auth cannot be used here — its session carries no `email` field at all, so it cannot exercise email-matching identity resolution; see "Employee identity and sign-in" in `packages/competence/INSTALL.md`, §7.)* Confirming this path needs an SSO login (Azure or Google) whose email matches a seeded employee record. The `docker compose` dev stack ships with `TI_WEB_AUTH_METHODS=local` only and no SSO credentials configured, so **this step cannot be verified on the dev stack as shipped** — it requires a separate environment with real Azure/Google OIDC credentials configured (INSTALL.md §7) and a seeded employee whose email matches the signing-in account.
