...
**No personal data crosses to the browser.** The preview projection carries counts and identifiers only. This is the same chokepoint the CLI already respects for stdout, extended to an HTTP payload.
...
- **Handlers** — a non-`admin` session is refused 403 on both services; the preview payload contains no personal field (assert against a fixture with distinctive names and emails, the way the CLI's no-leak proof is written); apply with a rejecting row writes nothing.
